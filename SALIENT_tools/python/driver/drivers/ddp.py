...
from SALIENT_tools.python.driver.drivers.base import BaseDriver
...
def get_ddp_config(job_dir: str, total_num_nodes: int,
    ...
    # Point1: Dist --ddp_dir(DDP_DIR=$OUTPUT_ROOT/$JOB_NAME/ddp)
    print("DDP dir is " + str(job_dir), flush=True)
    while True:
        # Point1: DDP_DIR内にある全ノード情報を取得。
...
class DDPDriver(BaseDriver):
    ...
    def __init__(self, args, device, dataset, model_type, ddp_cfg: DDPConfig):
        ...
        # Point1,Point2:各typeのデータセットのインデックスを渡し、各typeのDistributedShufflerを作成している。
