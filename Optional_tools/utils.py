...
runtime_stats_cuda = RuntimeStatisticsCUDA("Example")
...
# 統計データのKey群
rs_status_gl = {'total': 'Total', 'data_transfer': 'Data Transfer', 'sampling': 'Sampling + Slicing', 'train': 'Train'}

def get_rs_status():
    global rs_status_gl
    return rs_status_gl

def set_rs_status(status):
    global rs_status_gl
    rs_status_gl = status

def setup_rs(rs_name="RuntimeStatistics", performance_stats=True, stats=None):
    global runtime_statistics
    global performance_stats_enabled
    runtime_statistics = RuntimeStatistics("")
    ...
    if performance_stats:
        ...
        performance_stats_enabled = False
    # runtime_stats_cuda.name = " ".join([sampler, dataset_name, model_name])
...
def report_rs(logger=None):
    # if runtime_statistics is not None:
    #    runtime_statistics.report_stats()
    if is_performance_stats_enabled() and runtime_stats_cuda is not None:
        string_output = runtime_stats_cuda.report_stats(get_rs_status())
        if logger is not None:
            logger(('performance_breakdown_stats', string_output))
